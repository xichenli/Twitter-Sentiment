...
RE_PATTERNS = {
    ...
    ' url ':['(https?://[^\s]+)','(www\.[^\s]+)'],
    ' newspaper ':['R-G'],
    ' muslim ':['m[ou]sl[uieo][mn]','mulsim','musi'],
    ' american ':
        [
            'amerikan','u s','united states'
        ],
    ' fuck':
        [
...
def roc_auc_loss(
  ...
  with tf.name_scope(scope, 'roc_auc', [labels, logits, weights]):
    ...
    loss = tf.reduce_sum(tf.abs(labels_difference) * weighted_loss) * 0.5
...
def custom_loss(splitted_masks):
    def loss(y_true,y_pred):
        ...
        AUC_mean = tf.reduce_mean(AUC_array,axis=0)
...
# --------------------------------------------------------------------------------------------------#
train_df = pd.read_csv('../input/jigsaw-unintended-bias-in-toxicity-classification/train.csv',delimiter=',')
test_df = pd.read_csv('../input/jigsaw-unintended-bias-in-toxicity-classification/test.csv',delimiter=',')

#pre_tokenizer = PatternTokenizer()
#train_df["comment_text"] = pre_tokenizer.process_ds(train_df["comment_text"]).str.join(sep=" ")
#test_df["comment_text"] = pre_tokenizer.process_ds(test_df["comment_text"]).str.join(sep=" ")

for column in IDENTITY_COLUMNS:
    train_df[column] = np.where(train_df[column] >= 0.5, True, False)
train_df['target'] = np.where(train_df['target'] >= 0.5,1.0,-1.0)
...
for global_epoch in range(EPOCHS):
    x_train,train_mask,y_train = shuffle(x_train,train_mask,y_train,random_state=(global_epoch+1))
    model.fit(
        [x_train,train_mask],
        y_train,
...
predictions = (predictions-min_score)/(max_score-min_score)
